<template>
  <div class="nav">
    <ul v-infinite-scroll="loadMore"
      ...
      <li v-for="(item, index) in userSellHistoryProducts" :key="index">
  ...
  export default {
    ...
    methods: {
      getData(){
        ...
        this.$store.dispatch('findUserSellHistory', obj).then((res) => {
          if(res.data.list == ''){
            close.close();
            Toast({
              message: toast.userSellHistoryProduct,
              position: 'middle',
              duration: 1000
            })
          } else {
            this.userSellHistoryProducts = this.userSellHistoryProducts.concat(res.data.list);
