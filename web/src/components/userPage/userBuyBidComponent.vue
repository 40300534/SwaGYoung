<template>
  <div class="nav">
    <ul v-infinite-scroll="loadMore"
      ...
      <li v-for="(item, index) in userBuyBidProducts" :key="index">
  ...
  export default {
    ...
    methods: {
      getData(){
        ...
        this.$store.dispatch('findUserBuyBid', obj).then((res) => {
          if(this.userBuyBidProducts == ''){
            close.close();
            Toast({
              message: toast.userBuyBidProduct,
              position: 'middle',
              duration: 1000
            });
          } else {
            this.userBuyBidProducts = this.userBuyBidProducts.concat(res.data.list);
